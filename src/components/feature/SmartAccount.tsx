...
export default function SmartAccount({ mscaState }: { mscaState: TModularSmartAccountState }) {
  if (!mscaState.address || !mscaState.balance || !mscaState.tokenBalance) {
    return (
      <div className='w-full px-5 py-6 bg-[#1E1E1E] min-h-[335px] rounded-[16px]'>
  ...
  return (
    <div className='w-full px-5 py-6 bg-[#1E1E1E] min-h-[335px] rounded-[16px]'>
      ...
      <div className='mt-5'>
        ...
        <p className='text-white text-[32px] font-bold'>{mscaState.balance.slice(0, 6)} ETH</p>
      </div>
      <div className='mt-5'>
        <p className='text-[#646464] text-lg'>Token Balance</p>
        <p className='text-white text-[32px] font-bold'>{String(Number(mscaState.tokenBalance) / 1e6)} USDC</p>
      </div>
