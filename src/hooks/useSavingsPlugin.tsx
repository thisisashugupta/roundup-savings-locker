...
const useSavingsPlugin = () => {
  const { getTransactionReceipt, getUserOperationByHash } = useTransactionApis();
  ...
  const isSavingsPluginInstalled = async (extendedAccount: any) => {
    ...
    return installedPlugins.some((plugin: string) => plugin === savingsPluginAddress);
  };

  const getExtendedAccount = (modularAccountClient: any) => {
    // @ts-ignore
    const extendedAccount = modularAccountClient.extend(savingsPluginActions as unknown as any);
    console.log('Extended account address:', extendedAccount.account.address);
    return extendedAccount;
  };

  const createAutomation = async (extendedAccount: any, args: [Address, bigint]) => {
