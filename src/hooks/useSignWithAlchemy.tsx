...
import type { WalletClient, LocalAccount, SignableMessage, Hash, Address } from 'viem';
...
const useSignWithAlchemy = () => {
  ...
  const sendToken = async (extendedAccount: any, RECIPIENT: Address, AMOUNT: bigint) => {
    ...
    try {
      ...
      const response = txReceipt.logs
        .filter((log: { address: string }) => log.address.toLowerCase() === USDC.toLowerCase())
        .map((log: any) => ({
