...
import { installPlugin as installPlugin_, type Plugin, type FunctionReference } from '@account-kit/smart-contracts';
...
> = {
  updateOwners: (
    args: Pick<EncodeFunctionDataParameters<typeof MultiOwnerPluginExecutionFunctionAbi, 'updateOwners'>, 'args'> &
...
type ReadAndEncodeActions<TAccount extends SmartContractAccount | undefined = SmartContractAccount | undefined> = {
  ...
  encodeEip712Domain: (
    args: Pick<EncodeFunctionDataParameters<typeof MultiOwnerPluginExecutionFunctionAbi, 'eip712Domain'>, 'args'>,
  ) => Hex;

  readEip712Domain: (
    args: GetAccountParameter<TAccount>,
  ) => Promise<ReadContractReturnType<typeof MultiOwnerPluginExecutionFunctionAbi, 'eip712Domain'>>;

  encodeIsValidSignature: (
    args: Pick<EncodeFunctionDataParameters<typeof MultiOwnerPluginExecutionFunctionAbi, 'isValidSignature'>, 'args'>,
  ) => Hex;

  readIsValidSignature: (
    args: Pick<EncodeFunctionDataParameters<typeof MultiOwnerPluginExecutionFunctionAbi, 'isValidSignature'>, 'args'> &
      GetAccountParameter<TAccount>,
  ) => Promise<ReadContractReturnType<typeof MultiOwnerPluginExecutionFunctionAbi, 'isValidSignature'>>;
};

export type MultiOwnerPluginActions<
  TAccount extends SmartContractAccount | undefined = SmartContractAccount | undefined,
  TContext extends UserOperationContext | undefined = UserOperationContext | undefined,
> = ExecutionActions<TAccount, TContext> & ManagementActions<TAccount, TContext> & ReadAndEncodeActions<TAccount>;
...
export const MultiOwnerPlugin: Plugin<typeof MultiOwnerPluginAbi> = {
  ...
  getContract: <C extends Client>(
    client: C,
    address?: Address,
  ): GetContractReturnType<typeof MultiOwnerPluginAbi, PublicClient, Address> => {
    ...
    return getContract({
      ...
      client: client,
    }) as GetContractReturnType<typeof MultiOwnerPluginAbi, PublicClient, Address>;
  },
};
...
>(
  ...
  installMultiOwnerPlugin({ account = client.account, overrides, context, ...params }) {
    ...
    const pluginAddress = params.pluginAddress ?? (MultiOwnerPlugin.meta.addresses[chain.id] as Address | undefined);
