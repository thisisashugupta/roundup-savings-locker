...
> = {
  createAutomation: (
    args: Pick<EncodeFunctionDataParameters<typeof SavingsPluginExecutionFunctionAbi, 'createAutomation'>, 'args'> &
  ...
  pauseAutomation: (
    args: Pick<EncodeFunctionDataParameters<typeof SavingsPluginExecutionFunctionAbi, 'pauseAutomation'>, 'args'> &
      UserOperationOverridesParameter<TEntryPointVersion> &
      GetAccountParameter<TAccount> &
      GetContextParameter<TContext>,
  ) => Promise<SendUserOperationResult<TEntryPointVersion>>;
...
>(
  ...
  createAutomation({ args, overrides, context, account = client.account }) {
    ...
    if (!isSmartAccountClient(client)) {
      throw new IncompatibleClientError('SmartAccountClient', 'createAutomation', client);
    }
  ...
  pauseAutomation({ overrides, context, account = client.account }) {
    ...
    if (!isSmartAccountClient(client)) {
      throw new IncompatibleClientError('SmartAccountClient', 'pauseAutomation', client);
    }
  ...
  installSavingsPlugin({ account = client.account, overrides, context, ...params }) {
    if (!account) {
      throw new AccountNotFoundError();
    }

    if (!isSmartAccountClient(client)) {
      throw new IncompatibleClientError('SmartAccountClient', 'installSavingsPlugin', client);
    }
    ...
    const pluginAddress = params.pluginAddress ?? (SavingsPlugin.meta.addresses[chain.id] as Address | undefined);
